...
resource "aws_db_parameter_group" "this" {
  ...
  family      = "${var.engine}${regex("^\\d+", var.engine_version)}"
...
resource "aws_secretsmanager_secret_version" "db_credentials" {
  ...
  secret_string = jsonencode({
    ...
    engine   = var.engine
    host     = local.is_aurora ? "" : try(aws_db_instance.this[0].address, "")
    port     = local.is_aurora ? "" : try(aws_db_instance.this[0].port, "")
